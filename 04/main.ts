...
const clearTrailsButton = document.getElementById("but5") as HTMLButtonElement;
...
const mapCartesianToPolar = (x: number, y: number): [number, number] => {
...
let rdot = (r: number, th: number, mu: number) => {
    return r*(1-(r**2)) + mu*r*Math.cos(th)
    // return 0
}
let thdot = (r: number, th: number, mu: number) => {
    return 1
}
